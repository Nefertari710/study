...
## 第八天的内容
1. maven 介绍
	1. 定义：maven是一个项目生命周期管理工具, 是 apache的一个开源的项目
	2. 包括内容(对我们来说,我们更关心的是用它解决项目中的依赖问题)
		- 构建项目骨架
		- 清理
		- 编译
		- 测试
		...
		- 生成帮助站点
	3. 下载 
		- 地址: https://maven.apache.org/ ![](../图片资源/实习笔记图片资源/第八天/P1.png)
		- maven-3.8.5-bin.zip   8.35M大小![](../图片资源/实习笔记图片资源/第八天/P2.png)
	4. 安装
		- 直接解压到想安装的位置就可以
		- 要注意, 要使用maven,要先安装好JDK,并配置好环境变量 JAVA_HOME
	5. 把MAVEN 配置到环境变量中
		- 在环境变量中
			- 添加 D:\Program Files\apache-maven-3.8.5\bin; 这个路径就可以
		- 也可以用 MAVEN_HOME 的方式配置
			- MAVEN_HOME=D:\Program Files\apache-maven-3.8.5;
			- Path=%MAVEN_HOME%\bin;
	6. 在命令行中,执行 mvn -version 
		- 配置完成后效果![](../图片资源/实习笔记图片资源/第八天/P3.png)
		- 再执行命令 -> mvn help:system
			- 作用:输出系统相关信息![](../图片资源/实习笔记图片资源/第八天/P4.png)
	7.  (==重要的配置==)配置本地仓库的位置	
		- 默认情况下,它下载的内容,放在当前登录的用户的用户目录下的 .m2 中的 repository 中
		- 例如: 	C:\Users\Nefertari710\.m2\repository
		- 可以进行修改它的配置文件 
		    C:\Program Files\Apache Software Foundation\apache-maven-3.8.5\conf\settings.xml 
		- 中有如下配置:
		```xml
		 <!-- localRepository
				  ...
				  -->
		 <localRepository>C:/Users/Nefertari710/Documents/maven-repository</localRepository>
		```
		- 位置自己定-需要提前自己创建好maven-repository文件夹![](../图片资源/实习笔记图片资源/第八天/P5.png)
	8. 配置镜象仓库的位置
...
2. maven 项目的结构
	...
	3. 选择一个骨架类型 
		...
		- 输入包名  //可以随意输入, 我输入的是cat，可以直接跳过
		- 输入 Y  GAV  坐标,用于在仓库中唯一定位一个组件
3. 坐标
	1. GAV, 用来唯一标识一个组件在仓库中的位置
	2. 它的组成
	...
	3. 坐标的检索
		...
		- 输入需要查找的之后将下面生成的复制![](../图片资源/实习笔记图片资源/第八天/P6.png)
	4. 小常识:如何把项目中用到的依赖jar包复制出来--不能直接复制
	...
	5. 关于项目生命周期管理的几个常用 Maven 命令mvn clean   //清理 ,清理上次生成的内容
		- mvn compile //编译
		- mvn test  //运行测试
		- mvn package  //打包
		- mvn install  //把项目布署到本地仓库中 
			- 可以在之前我们创建的maven-repository中找到
			- 在后续的程序中就可以使用了--在依赖处添加部署到本地仓库项目的groupid/artifactId/version
			- 我们安装到本地仓库之后，可以在其他的maven项目中通过添加依赖的的方式将内容引入到新项目中，即使我们关闭了项目依旧可以使用。
		- mvn site //生成相关的帮助网站
	6. 我们可以在ecplise中执行maven命令
		- 在项目上,右键 选 Run As ,选第二个Maven Build ,然后在上面的 Goals 对应的框中输入相关的命令即可
		- 比如输入
		    - clean
		    - claen package 先clean再package
		    - clean install  
		    - 但要注意,不要写成 mvn clean
4. 在eclipse中如何配置maven
	1. 链接![](eclipse中如何配置maven)
	2. 在eclipse中配置maven路径 
		- window-> preference --Maven --> installation
	    - 点右边的Add, 找到maven的安装位置,添加maven, 添完以后,一定不要忘了把它前面的==复选框勾选上==
    2. window-> preference --Maven --> User Settings 
	    1. 把上面的Global Settings 和 下面的 User Settings 都指向maven的配置文件setting.xml就可以
		2. 在建maven项目的时候,经常会出现由于组件没有下载下来而出错的情况	
			- 出现这种情况,我们可以重新更新几次项目
			- 方式是在工程上,右键 Maven  - Update Project
			- 更新步骤1![](../图片资源/实习笔记图片资源/第八天/P7.png)
			- 更新步骤2![](../图片资源/实习笔记图片资源/第八天/P8.png)
		3. 有时候,多次更新也无效  会出现  ...  was cached in local Repository ....
			- 可以去本地仓库中,把这些 缓存的相关信息清理掉
			- 到本地仓库中执行 
				- 在\maven-repository目录下直接打开命令行
				- 执行 for /r %i in (*.lastUpdated) do del %i   
				- 作用是清理所有以 *.lastUpdated 结尾的文件
		4. 如果以上方法还解决不了,可把把本地仓库删除掉,换别人的,好用的上来
5. 在eclipse中创建 mavenweb 项目
	1. 链接![](在eclipse中创建mavenweb项目)
	2. 创建一个maven工程,类型选择maven-archetype-webapp
	3. 我们发现它默认的JDK是 1.5 ,可以配置插件进行管理-配置以后,要进行一下更新 
	```xml
	<build>
		<plugins>
			<plugin>
	...
	5. 我们在index.jsp中写入el表达式,发现它不能解析
		...
		${msg }
		```
		2. 发布项目到tomcat上,访问  http://localhost:8080/web-system/index.jsp
			- 发现没有解析 ${msg},而是按原样输出了
		3. 我们可以把web.xml中的内容进行一下修改,把它换成下面的样子
		```xml
		<?xml version="1.0" encoding="UTF-8"?>
...
1. git简介
	1. 版本库（version control system)
		- 版本控制系统（VCS）是一种记录一个或若干文件内容变化，以便将来查阅特定版本修订情况的系统。使用版本控制系统通常还意味着，就算乱来一气把整个项目中的文件改的改删的删，你也照样可以轻松恢复到原先的样子。 但额外增加的工作量却微乎其微。常见的版本库 SVN ,GIT ,VSS 
    2. 集中式版本库和分布式的版本库 
	    1. 集中式:如SVN,CVS等
		    1. 介绍: 有一个集中管理的服务器，保存所有文件的修订版本。而协同工作的人们都通过客户端连接到这台服务器，获取最新的文件或者提交更新. 多年以来，这已成为版本控制系统的标准做法。
		    2. 优点
			    - 逻辑明确，符合一般人思维习惯
				- 易于管理，集中式服务器更能保证安全性
				- 代码一致性非常高
				- 适合开发人数不多的项目开发
			3. 缺点
				- 单点故障:如果中心数据库所在的位置发生损坏，又没有做恰当备份,你将丢失所有数据——包括项目的整
			      个变更历史,只剩下人们在各自机器上保留的单独快照。
		2. 分布式版本控制系统: 如GIT
			1. 介绍: 客户端并不只提取最新版本的文件快照，而是把代码仓库完整地镜像下来.任何一处协同工作用的服务器发生故障，事后都可以用任何一个镜像出来的本地仓库恢复。因为每一次的克隆操作，实际上都是一次对代码仓库的完整备份.
			2. 优点
				- 适合分布式开发，强调个体
				- 公共服务器压力和数据量都不会太大
				- 速度快、灵活
	...
	3. GIT
		1. 介绍: 开源的分布式版本控制系统，用C语言编写Git 与其他的版本控制工具 CVS, SVN，VSS相比，它采用了分布式版本库的方式，可以离线提交代码，代码保存在本地，可以享受SVN联网能进行的一系列操作，如果你需要推送到服务器只需要push一下即可，每个人都是一个独立的仓库。
		2. Git和SVN的区别:
			- Git 是分布式的，SVN 不是
			- Git 把内容按元数据方式存储，而 SVN 是按文件
			- Git 分支和 SVN 的分支不同：分支在 SVN 中实就是版本库中的另外一个目录
			- Git 没有一个全局的版本号，而 SVN 有
			- Git 的内容完整性要优于 SVN：Git 的内容存储使用的是 SHA-1 哈希算法
2. git的安装
	1. 官网下载地址: https://git-scm.com 
	2. 实习使用的版本: Git-2.24.1.2-64-bit.exe
	3. 注意: 实际上,在安装过程中进行的一些选择
		- 在 C:\Program Files\Git\etc文件中也是可以进行修改的![](../图片资源/实习笔记图片资源/第九天/P1.png)
	4. 打开git命令行的方式
		- 右键 ,在弹出菜单中点  Git Bash Here![](../图片资源/实习笔记图片资源/第九天/P2.png)
	    - 直接用命令行窗口输入git![](../图片资源/实习笔记图片资源/第九天/P3.png)
3. git bash命令行
	1. 常见技巧
		- 可以按住ctrl键 ,滚动鼠标滚轮,调整字体大小
		- 可以用 tab 键进行补全
		- 可以用 上下箭头翻看执行过的命令
	2. 常用指令
		- ls  //列出当前目录下的文件
		...
		- cat  //查看文件内容
	3. vim 文本编辑器的使用
		- vim 文件名 进入指令模式,在这个模式下,不能进行编辑
		- 按下 i 字符,进入编辑模式,可以进行编辑
		- 编辑完成以后,按esc,进入指令模式
		- 指令模式下输入 :wq 保存退出  q! 表示强制退出
4. 仓库的创建与提交
	1. git 的全局配置
		1. 指令--设置全局的名字和邮箱
			- git config --global user.name "Nefertari"
			- git config --global user.email "2583741979@qq.com"
			- 图示![](../图片资源/实习笔记图片资源/第九天/P4.png)
		2. 要注意 user.name 和  user.email 是固定写法,不要写错 
			- 这个用户名和邮箱名,主要用于标识是哪个用户
			- 邮箱格式要求不严
		3. 配置完以后,可以用下面的命令查看
			- git config --global -l 
			- 效果![](../图片资源/实习笔记图片资源/第九天/P5.png)
		4. 上面的配置是在用户文件夹(我的电脑是 C:\Users\Nefertari710 ) 中的 .gitconfig 文件中添加了以下内容![](../图片资源/实习笔记图片资源/第九天/P6.png)
	2. 创建仓库,完成版本的控制
		1. git中的工作区,暂存区,历史区,远程仓库
			1. 职责划分
				- 工作区: 我们当前工作的目录
				- 暂存区: 英文 stage 或index 
				- 历史区: 放的就是接受管理的版本了
			...
			3. 下载的流程
				- 远程仓库 => 工作区 
		2. 创建本地仓库
			- 新建一个文件夹作为你的工作区后执行指令: git init 
			- 注意: 执行完毕以后,会在当前目录下生成一个叫 .git 的隐藏目录,千万不要删除,暂存区,历史区,和其他的一些信息都放在这里![](../图片资源/实习笔记图片资源/第九天/P7.png)
		3. 编写文件,然后提交到暂存区
			- touch a.java
		    - touch b.java
			- touch a.html
			- touch b.html
			- 前四条执行效果指令:ll![](../图片资源/实习笔记图片资源/第九天/P8.png)
			...
			- git status  
				...
				- 图示:![](../图片资源/实习笔记图片资源/第九天/P9.png)
		4. 提交到历史区
			- 指令: git commit -m "这里要添加一些说明信息"
			- 解释: -m 参数后面跟的是一些说明信息,信息和-m之间可以有空格也可以没有![](../图片资源/实习笔记图片资源/第九天/P10.png)
			- 注意: 如果提交的时候,不指定 -m参数,它会在提交的时候默认打开vim编辑器,我们可以在这里输入注释以后,再进行提交
		5. 查看历史区中的版本
			1. git log 查看各个版本![](../图片资源/实习笔记图片资源/第九天/P11.png)
			2. git log --pretty=oneline  查看各个版本,以精简的方式显示 ![](../图片资源/实习笔记图片资源/第九天/P12.png) 
		6. 如何把提交到暂存区的内容撤销
			- 前提: 没有进行提交
...
5. 在git中忽略文件
    ...
    - 图示:
	    - 创建vim .gitignore![](../图片资源/实习笔记图片资源/第九天/P13.png)
		- 常见的内容 ![](../图片资源/实习笔记图片资源/第九天/P14.png)
		- 效果就是再次输入git status指令的时候就不会显示隐藏内容了              

## 第十天的内容
1. git中的文件比较
	1. 关于git中的中文文件名显示
		- 默认情况下,用git管理的文件名如果是中文,在使用命令的时候它显示的是一堆码值![](../图片资源/实习笔记图片资源/第十天/P1.png)
		- 可以设置:
			- 指令: `git config --global core.quotepath false`
			- 设置后的效果![](../图片资源/实习笔记图片资源/第十天/P2.png)
	2. 文件比较
		- 指令: git diff
		- 功能: 用于比较当前工作区中文件的的内容,和以往提交的内容有什么不同
		- 示例: 建一个叫词.txt的文件,内容如下
		```txt
		在遥远的西世界
		有一个川国
		他是一位中国人
		...
		因为他去了柬埔寨
		```
		- 执行指令: `git diff`![](../图片资源/实习笔记图片资源/第十天/P3.png)
2. git版本回退，和checkout指令
	1. 撤销提交到暂存区的内容
		- 指令
		```git
		...
		```
		- 效果:![](../图片资源/实习笔记图片资源/第十天/P4.png)
	2. 历史版本的回退
		1. 查询提交后退的历史纪录
			- 代码: `git reflog`
			- 效果: ![](../图片资源/实习笔记图片资源/第十天/P5.png)
			- 查看简要信息时后面加上参数: --pretty=onelin
			- 效果：![](../图片资源/实习笔记图片资源/第十天/P6.png)
		2. 相关回退指令
			- 切记不可以删除.git文件，因为所有的历史记录全部都存储在.git文件中，如果删除了.git文件历史版本就对丢失
		```git
		git reset --hard HEAD^ //回滚到上一次版本		    
		git reset --hard 6e8e565dd3c5292f0474d3537bf05f2425c4c4e7 
		git reset --hard 6e8e5 //效果和上面相同,这个号可以写的短点
		```
	3. checkout签出
		- 代码
		```git
		...
		```
		- 效果:![](../图片资源/实习笔记图片资源/第十天/P7.png)
		- 解释: 上述的代码我们将test.txt文件从主机端删除了，但是由于我们并没有在git上进行提交，所以我们可以使用这个chekout实现一种类似于文件恢复的工作
	4. 删除版本库中的某个文件
		- 操作方法: 先在工作目录中删除要删除的文件，之后进行提交就相当于把库中的文件删除了
		- 代码
		```git
		git add .
		git commit -m"Nefertari删除了test.txt"
		```
		- 注意: 当我们将版本库中的文件删除之后就没有办法再进行签出操作了
3. github 的使用
	1. 官网  https://github.com
	2. 介绍: GitHub是一个面向开源及私有软件项目的托管平台，因为只支持 git 作为唯一的版本库格式进行托管，故名GitHub。
	3. 使用流程
		1. 注册账号 (邮箱验证)
		2. 激活账号
		3. 设置个人信息
		...
		6. 可以添加合作者 Collaborators
			- 会给对方发一个邮件,发到对方邮箱里(不是github账号),对方要登录邮箱,同意一下才行，被邀请者,要先登录github
		7. 如果仓库不用了,可以删除仓库
4. 码云的使用
	1. 官网: https://gitee.com
	2. 介绍: 码云相当于中国的github, 码云也支持 SVN 
	3. 特点: https://gitee.com/contrast
		- 列出了码云和git本比的一些特点
	4. 社区版容量说明: 
	...
	5. 在码云上创建一个仓库 (本例名为 black-shop) 
		1. 右上角的加号选择创建仓库![](../图片资源/实习笔记图片资源/第十天/P8.png)
		2. 对应的信息填入![](../图片资源/实习笔记图片资源/第十天/P9.png)
		3. 根据引导创建相关内容例如readme.md![](../图片资源/实习笔记图片资源/第十天/P10.png)
	6. 如何把本地仓库和远程仓库关联起来?
		1. 在本地建一个目录名称任意 比如我的叫Local_Repository
		2. 在目录上,右键, Git Bash Here
		3. 执行命令: `git init` 
			- 代码段 ![](../图片资源/实习笔记图片资源/第十天/P11.png)
			- 执行效果：在该文件夹中生成.git文件夹
		4. 用命令和远程仓库关联
			- 代码段
			```git
			git remote add origin https://gitee.com/Nefertari710/black-shop.git    //origin相当于给远程仓库起了个名
			git remote -v //查看本地仓库和远程仓库的连接信息
			```
			- 执行效果:![](../图片资源/实习笔记图片资源/第十天/P12.png)
			- 注意: 查看连接信息给出的两行，一个是下载，一个是上传
		5. 从远程仓库拉去内容
			1. 指令: `git pull origin master`	 
				- 解释: git pull 命令,用于从远程仓库拉取内容 origin 就是远程仓库地址的别名 , master 表示分支名
				- 这个命令,可以直接简写成 `git pull`
				- 第一次执行的时候出现了 fatal: couldn t find remote ref master,原因是我们创建的git仓库中还没有任何内容,也不存在master分支,可以在git上创建个readme或其他文件,就可以执行上述指令了
				- 效果:![](../图片资源/实习笔记图片资源/第十天/P13.png)
			2. 再执行指令：`git pull origin master`
				- 问题: 出现一个错误fatal: refusing to merge unrelated histories![](../图片资源/实习笔记图片资源/第十天/P14.png)
				- 原因: 本地仓库和远程仓库是两个独立的仓库, 我们可以采用强制合并的方式合并
				- 解决方法:
					- 指令: `git pull origin master  --allow-unrelated-histories`
					- 会弹出一个窗口让输入备注信息-内容随意![](../图片资源/实习笔记图片资源/第十天/P15.png)
					- 输入备注信息后![](../图片资源/实习笔记图片资源/第十天/P16.png)
		6. 在本地编写内容,往远程提交```
			- 指令: `git push origin master`
			- 指令执行效果: ![](../图片资源/实习笔记图片资源/第十天/P17.png)
			- 远程仓库效果![](../图片资源/实习笔记图片资源/第十天/P18.png)
		7. 如果想去掉和远程仓库的关联
			- 指令: `git remote rm origin`
		8. 在实际工作中,我们不要这么麻烦,可以直接克隆下来用就行
			1. 指令: `git clone [仓库的地址]`
			2. 可以说,git clone直接解决了以下几个问题
				- 初始化了仓库
			    - 直接建立本地仓库和远程仓库的关联
			    - 把远程仓库的内容,下载到了本地
5. git中的分支管理
	1. 分支的查看
		1. 方法一 -- 查看分支
			- 指令: `git branch`
			- 输出: ![](../图片资源/实习笔记图片资源/第十天/P19.png)
			- 解释:  * master 默认只有master分支,前面带有 * 代表它是当前工作的分支
		2. 方法二 -- 查看所有分支
			- 指令: `git branch -a`
			- 解释: -a这个参数表示显示所有分支,包括本地分支和远程分支
			- 输出:![](../图片资源/实习笔记图片资源/第十天/P20.png)
	2. 创建分支
		- 指令: git branch dev  //创建一个分支,叫dev
		- 输出: ![](../图片资源/实习笔记图片资源/第十天/P21.png)
	3. 切换分支				  
		1. 切换分支: `git checkout dev`  
			- 功能: 切换到dev分支
			- 输出: ![](../图片资源/实习笔记图片资源/第十天/P22.png)
		2. 查看当前分支: `git branch` 
			- 输出: ![](../图片资源/实习笔记图片资源/第十天/P23.png)
			- 经过了上面的切换指令我们再查看当前分支的时候，星号就出现在切换的分支dev上了
		3. 直接创建并切换到这个分支
			- 指令: `git checkout -b dev1` 
			- 效果![](../图片资源/实习笔记图片资源/第十天/P24.png)
	4. 删除分支 
		- 指令: `git branch -d dev`
		- 效果:![](../图片资源/实习笔记图片资源/第十天/P25.png)
		- 注意: 必须切换到其他分支上才能删除, 不能自已删除自已,这个分支只是删除远程分支
	5. 合并分支
		1. 创建一个叫dev的分支,并切换到这个分支,然后这个分支上做一些提交
		2. 代码:
		```git
		git branch dev;    //创建dev分支
		git checkout dev;  //切换到dev分支
		touch dev_a.txt;   //创建文件dev_a
		touch dev_b.txt;   //创建文件dev_b
		git add .          //进行提交上传，并且备注上传信息
		git commit -m "小李在dev分支上添了两个文件"		
		```
		3. 由于上面的操作是在dev分支上进行的,所以master分支上是看不到的
			1. 首先在dev分支下查看提交信息`git log`![](../图片资源/实习笔记图片资源/第十天/P26.png)
			2. 转换到master分支下再次查看`git log`![](../图片资源/实习笔记图片资源/第十天/P27.png)
		4. 可以用命令,将dev分支,合并到master分支上
			- 切换分支指令: ```git checkout master```   
				- ==注意: 合并前,要先切换到master分支上==
		    - 合并分支指令: `git merge dev`  
			    - 输出效果: ![](../图片资源/实习笔记图片资源/第十天/P28.png)
				- 解释：上面的指令执行后dev分支就被合并到master分支上了，当我们在合并后的master分支上再次查看提交日志的时候就可以查看到之前在dev分支上的上传日志了	
	6. 冲突的处理
		1. 创建一个新的分支 admindev,然后修改某个文件,比如修改 login.jsp, 并提交
		2. 代码:
		```git
		git branch admindev
		...
		```
		4. 进行合并`git merge admindev`
			- 提示: Auto-merging login.jsp
			...
			```
			- 它把双方对这个文档内容的修改,都进行了标记和保留,我们的处理方式,手动把这个文档改成我们想要的样子,比如
			```js
			<form>
			...
			```
	7. 把本地的分支推到远程上
		1. 代码: `git push origin admindev`
			- 这时我们再到 github 上去查看,就能看到刚才推上去的分支
		2. 代码: `git checkout admindev`
		3. 创建 .或修改一些文件 ...
		```git
		git add .
		...
		```
		4. 这时我们再到github上查看,可以看到 admindev 分支上推上来的内容
	8. 在本地删除远程分支
		1. 代码: `git branch -a`
		2. 输出* admindev
					  dev
		...
		3. 代码: `git push origin -d  admindev`
			输出: remote: Powered by GITEE.COM [GNK-6.3]
...
7. git 团队协作与代码冲突
	...
	2. 小李也进行了一些修改,然后
		...
		- 解释: 它的意思,是由于其他人对版本库进行了提交, 我们需要更新以后再进行push
		- 代码: `git pull `
			- 输入上面的命令以后,它会进入vim编辑器,让我们编写备注信息,我们可以按具体的情况编写,保存
		- 代码: `git push`
    3. 冲突的发生
	    1. 小张修改 pom.xml  ,比如修改了下面的内容
		...
		5. 提示内容:
			To https://gitee.com/fangyiguo/aolin-system.git
						...
						hint: (e.g., 'git pull ...') before pushing again.
						hint: See the 'Note about fast-forwards' in 'git push --help' for details.
			- 解释:它是告诉我们要先进行 git pull		
		6. git pull	
			1. 报错:error: Pulling is not possible because you have unmerged files.
					hint: Fix them up in the work tree, and then use 'git add/rm <file>'
					hint: as appropriate to mark resolution and make a commit.
					fatal: Exiting because of an unresolved conflict.
			2. 上面的意思就是指发生了冲突,我们要进行修正冲突,我们可以编辑pom.xml 文件,把冲空的地方处理掉
			3. 处理完以后,git add ,git commit ,git push 就可以了