...
3. Spring示例
	...
	3. 测试
		...
		```
		- 测试结果![[$GZOT$X3PTY~9WCF]5N6$0T.png]]
...
4. setter方式注入
	...
	3. 我们只研究setter方法注入
		...
		5. 测试
			...
			```
			 -  测试结果![[[PZ`2UN7IW_]A@5DFMVFEC3.png]]
		...
		1. 测试新的Oracle实现类
			...
			```
			- 测试结果![[X0E8KZ$LB6S4TQ5{UAOF1RS.png]]
5. bean的作用域
	...
	2. 当然Spring中管理bean，有几种作用域(scope的取值)
		1.  singleton: 是默认值，表示创建的是单实例 - 默认情况不写
			...
			```
			6. 单例例子
				- 配置文件![[EA57`7~EQ%79)%B9TPJIV87.png]]
				- 测试文件![[1E%E9VV8S0D9X]I%QX_RVBP.png]]
				- 测试结果 true  
		2. prototype
			- 这个取值，表示每次都将产生一个新的对象
			...
			- 多例例子
				- 配置文件![[[QZ8YIM828VT@OI93RQ%0`B.png]]
				- 测试文件![[PHAC[{]~T9E((33TDI(YIZE.png]]
				- 测试结果false
		3. request(不做细化介绍)
		4. session(不做细化介绍)
6. 集合类型的装配
	...
	- map集合
		...
		```
		- 注入后的输出效果 ![[{U@N35P~WY2N{EN`@[MY8{O.png]]
7. p名称空间
	- 先需要选中p名称空间![[T6[)Q`R0)E)HRB2LUVC1F[0.png]]
	- 选中后bean可以按下面的方式写
	```xml
...
3. 自动扫描
	...
	1. 引入context名称空间
		- 在Namespaces处选中context![[_)XX2RV]I8X3)KQ64T$AB%1.png]]
		- 内容
		```xml
		<?xml version="1.0" encoding="UTF-8"?>
		<beans xmlns="http://www.springframework.org/schema/beans"
		...
		```
		- 引入之后配置文件发生变化![[AUYXR]`T0$HOLP84P4)UW6G 1.png]]
	2. 开启自动扫描
		...
		- 这些类必须带有以下注解之一
			- @Controller 用于控制层
				- 注解样式![[43Y@QQV]VEVMN1@SRSM45$M.png]]
			- @Service 用于业务层
			- @Repository 用于持久层
				- 注解样式![[R)G]4[@IJ8(XDHZH3Q)NYHO.png]]
			- @Component 用于其他组件
		- 以自动扫描的方式纳入spring管理的beans，有默认名称，默认名称就是类型小写
			- 默认就是小写![[SGR8`J~(3F~7EUXT(~[PVVU.png]]
			- 输出结果![[FA95S7[BM`$({79`YF]`E30.png]]
		- 我们也可以自己指定名称
			- 代码段
			```java
			@Repository("daoMysql")
			...
			```
			- 实例![[S)DMJ}HD(})U}VPEE5K23`V.png]]
			- 输出结果![[U9Q0UXNIYPH0M[TS{$Q7K7V.png]]
		- 用注解方式管理bean默认的单实例，如果想配置成多实例
			...
			```
			- 示例![[IMKAX`N1STL`~B}5Q9_QS}S.png]]
			- 示例![[Y{{[T0LJI0@5M@$1%SJ$}T0.png]]
			- 输出对比![[H(%@TN9Y8)T86S0~G)139D3.png]]
4. 注解装配
	1. 解决的是依赖注入的问题
	```java
	package com.controller;
...
6. Spring中进行AOP编程
	...
	1. 注解方式 
		1. 导入相关的包 
			- 可以用maven进行依赖的获取![[Pasted image 20220422122645.png]]
		2. 配置文件
			- 打开AOP名称空间![[JGUX{1B(A5J1}PN3`O%ODDY.png]]
			- 然后对配置文件进行修改
				- 代码段
				```xml
				<aop:aspectj-autoproxy />
				```
				- 图示![[ROY)WD]J[)R`665IX%DW[%W.png]]
		3. 接口
		```java
		public interface IUserDao {
			void addUser();
...
2. mybatis 做为dao层
	...
	4. 使用mapper代理的方式开发持久层 
		1. 规则
			1. 映射文件中的namespace 属性,要写成接口的全路径名称![[{SQEBC{_3WCDHK`]_WP)${P.png]]
			2. mapper接口中声明的方法名,要和映射文件中的sql的id相同![[LK063~@I@~D{H%)D~@@BE_U.png]]
			3. 接口中方法的返回值,要和sql中的 resultType 一致![[YZNXXL(ZM~AIGNK))`AI_R5.png]]
				- 对于增删改本身没有resultType这个属性
			4. 接口中的方法的参数类型,要和sql中的 parameterType 一致![[SDY_%WGL_Q{$%16[_BQTEG6 1.png]]
		...
		5. 测试
			...
			```
			- 测试结果
				- 测试之前![[S)M9S[P785NSE_6L7MQIQ]A.png]]
				- 测试过程![[TF}B~L3@X1P)02I00`N{SAD.png]]
					- 删除失败是因为我们删除了一个不存在的id
				- 测试结果![[[ND`SIYS~QXLF2_V00QCF7Y.png]]
3. 映射文件的引入方式 
	- 在主配置中引入映射文件
	1. 相对于类路径的方式
	```xml
	...
	3. 使用类名,包名的方式 -- 用的不是太多
		- 要用这种方式,则要求映射文件的名称和接口名要一致,且位于同一包下![[QELP]ZH3X2YFT8[W(PLL_SR.png]]
		- 代码内容
		```xml
		<mappers>
...
4. 传入多个参数
	...
	2. 我们也可以在声明接口的时候,用注解的方式传入多个参数
		...
		```
		- 输出结果![[Pasted image 20220426191305.png]]
5. mybatis 配置输出sql语句
	...
	3. 这样在运行的时候,它会输出运行的sql语句,如下:
		...
		```
		- 输出效果![[Pasted image 20220426192055.png]]
6. 动态sql
	...
	3. 例 mybatis中做动态查询 ( if 的使用 )
		...
		- 测试
			...
			```
			- 没有任何限制的时候![[Pasted image 20220426200729.png]]
			- 限制note=simple![[Pasted image 20220426200842.png]]
			- 限制note=simple且password=133![[Pasted image 20220426201114.png]]
			- 限制note=simple且password=133且username=admi![[Pasted image 20220426201359.png]]
	...
	5. foreach的使用
		...
		3. 测试类中调用
			...
			```
			- 测试结果![[Pasted image 20220426203930.png]]
		4. 日志
		```java
	...
	6. 实在不行,我们可以用下面的方法处理复杂的查询
		...
		userList.forEach(System.out::println);
		```
		- 输出结果![[Pasted image 20220426205012.png]]
		- 相当于直接传sql语句进行查询
		- 切记：==防止用户非法注入==
7. sql片
	1. 使用sql片段示例1
	...
	2.  使用sql片段示例2 -- 主要看id= “cols”
		...
		```
		- 输出结果![[H90EOP{NQCB13Q$NV{KD%LU.png]]
8. 注解的方式
	- 接口
	```java
	public interface UserMapper {
...
2. 配置核心控制器
	...
	```
3. springmvc 主配置文件
	- 在源文件夹下 (比如src) 建 springmvc-config.xml ![[4%DU[%{X(WZ{%P}8{~FV})2.png]]
		- 依旧是new中选择other之后输入bean	
		- 依旧需要在Namespace中开启context自动扫描和mvc![[5[B8LRRYY5WY}([JHD[4F%2.png]]
	- 内容
	```xml
	<?xml version="1.0" encoding="UTF-8"?>
	<beans xmlns="http://www.springframework.org/schema/beans"
...
1. 参数绑定与值的传递
	...
	3. pojo类型--普通类
		...
		3. 页面 WEB-INF/view/user-add.jsp
			...
			${msg} 
			```
			- 页面效果![[Pasted image 20220504034535.png]]
			- 提交会有网页提示窗口![[Pasted image 20220504034640.png]]
			- 原界面效果![[W$5F7RA2RH[W%5O405G6ETW.png]]
			- 通过访问localhost:8080/springmvc-06/user/getAll可以看到成功加入![[ZW0IV)5W`DSL~CFPEE~V3@Q.png]]
		4. 控制层中的添加方法
			- 代码段
			```java
			//注意,它只处理POST请求	
		...
		5. 附: 关于日期类型的处理
			...
			```
			- 启动,运行,添加的时候,生日填入 2000-12-20 提交,发现出现了400 问题其实,400是在用springmvc框架时常见的错误,主要是提交的请求数据类型和服务端想要的数据类型不匹配造成的(比如服务端想要数值类型,但客户端提交的是非法字符串)![[Pasted image 20220504040606.png]]
			- 处理方式: 在实体类中的字段上用注解--可以不填
				...
				private Date birthday;
				```
				- 处理后打印端格式![[00SM(MF~WLN}@HJSP5DWM5M.png]]
			- 月份是1-9月都要加0变成01-09月
...
6. restfull风格
	...
	3. 关于 Representational State Transfer (表现层状态转化) 的理解
	    ...
	    3. State Transfer 状态转化
			...
			- 界面会是404但是有输出效果![[UGU3GS(Z3]3)P%RNKG~K%26.png]]
7. 文件上传
    1. 配置文件 
	    - 代码段
		```xml
