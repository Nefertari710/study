...
4. 仓库的创建与提交
	1. git 的全局配置
		...
		4. 上面的配置是在用户文件夹(我的电脑是 C:\Users\Administrator ) 中的 .gitconfig 文件中添加了以下内容
		```gitconfig
		[user]
			name = cat
	...
	2. 创建仓库,完成版本的控制
		...
		2. 创建本地仓库
			- 指令: git init 
			- 注意:执行完毕以后,会在当前目录下生成一个叫 .git 的隐藏目录,千万不要删除,暂存区,历史区,和其他的一些信息都放在这里
		3. 编写文件,然后提交到暂存区
			- touch a.java
		    - touch b.java
			- touch a.html
		...
		4. 提交到历史区
			...
			- 解释: -m 参数后面跟的是一些说明信息,信息和-m之间可以有空格也可以没有
			- 注意: 如果提交的时候,不指定 -m参数,它会在提交的时候默认打开vim编辑器,我们可以在这里输入注释以后,再进行提交
		5. 查看历史区中的版本
			1. git log 查看各个版本
			2. git log --pretty=oneline  查看各个版本,以精简的方式显示  
